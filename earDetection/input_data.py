...
def get_batch(image,label,batch_size,crop_size):
	...
	images,label_batch=tf.train.shuffle_batch([distorted_image,label],batch_size=batch_size,num_threads=2,capacity=200,min_after_dequeue=100)
	#调试显示
...
def get_test_batch(image,label,batch_size,crop_size):
	...
	distorted_image=tf.random_crop(image,[crop_size,crop_size,3])#随机裁剪
	images,label_batch=tf.train.batch([distorted_image,label],batch_size=batch_size,allow_smaller_final_batch=True)
	return images,tf.reshape(label_batch,[batch_size])



def test1():
	encode_to_tfrecords("data/val.txt","data",new_name='val.tfrecords',resize=(256,256))


def test():
	#encode_to_tfrecords("data/val.txt","data",new_name='val.tfrecords', resize=(256,256))
	image, label = decode_from_tfrecords('data/val.tfrecords')
	pdb.set_trace()
	batch_image,batch_label = get_batch(image,label,32,227)#生成测试的样例batch
